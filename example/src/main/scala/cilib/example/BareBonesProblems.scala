...
object BarebonesProblems {
  ...
  def starTask(prob: ProblemDef, seed: Long): Task[Maybe[Double]] = Task {
    ...
    val sync = Iteration.sync(barebones)
  ...
  def starLineTask(prob: ProblemDef): Task[String] = {
    ...
    taskAvg.map(avgs => s"$strategy,$iterations,$repeats,${prob.name},$dim,$avgs")
  }
  ...
  def writeOut(lines: List[String]) = {
    printToFile(new java.io.File(s"${output}/${strategy}.txt")) { p =>
      p.println(s"Strategy,Iterations,Repeat,ProblemClass,Problem,Dimension,avgs")
