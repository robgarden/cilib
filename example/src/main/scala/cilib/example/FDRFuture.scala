...
object FDRFuture extends SafeApp {
  ...
  val output = "/home/robertgarden/Dropbox/results/fdr1"
  ...
  def ferFutureLine(w: Double, c1: Double, c2: Double, probClass: String, prob: ProblemDef): Future[String] = {
    val futures: Future[List[Maybe[Double]]] = Future.sequence((0 until repeats).toList.map(i => fdrFuture(w, c1, c2, prob, i.toLong)))
