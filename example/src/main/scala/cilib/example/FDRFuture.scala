...
object FDRFuture extends SafeApp {
  import java.io._
  ...
  val output = "/home/robertgarden/Dropbox/results/fdr"
  ...
  def fdrFuture(w: Double, c1: Double, c2: Double, c3: Double, prob: ProblemDef, seed: Long): Future[Maybe[Double]] = Future {
      val cognitive = (Guide.pbest[Mem[List,Double],List,Double], c1)
      val social = (Guide.gbest[Mem[List,Double],List], c2)
      val fdr = (Guide.fdr[Mem[List,Double]], c3)

      val fdrPSO = constrictionPSO(w, List(cognitive, social, fdr))
  ...
  def ferFutureLine(w: Double, c1: Double, c2: Double, c3: Double, probClass: String, prob: ProblemDef): Future[String] = {
    val futures: Future[List[Maybe[Double]]] = Future.sequence((0 until repeats).toList.map(i => fdrFuture(w, c1, c2, c3, prob, i.toLong)))
    val futuresAvg: Future[Double] = futures.map(lf => lf.sequence.map(l => l.sum / l.length).getOrElse(-666))
    futuresAvg.map(avg => s"$strat,$iterations,$repeats,$probClass,${prob.name},$dim,$w,$c1,$c2,$c3,$avg")
  }

  def problemFuture(probClass: String, prob: ProblemDef): Future[List[String]] = {
    val futureLines: List[Future[String]] = for {
      (w, c1, c2, c3) <- params
    } yield ferFutureLine(w, c1, c2, c3, probClass, prob)
  ...
  def writeOut(s: String, probClass: String, name: String, lines: List[String]) = {
      printToFile(new File(s"$output/${s}_${probClass}_${name}.txt")) { p =>
        p.println(s"Strategy,Iterations,Repeat,ProblemClass,Problem,Dimension,w,c1,c2,c3,avgbest")
