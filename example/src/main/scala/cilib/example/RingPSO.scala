...
object RingPSO extends SafeApp {
  import java.io._
  ...
  val output = "/Users/robertgarden/Desktop/results/ring"

  val cognitive = Guide.pbest[Mem[List,Double],List,Double]
  val (strategy, guide) = ("ring", Guide.lbest[Mem[List,Double],List](3))
  ...
  problemsClasses.foreach { case (probClass, problems) =>
    ...
    problems.foreach { prob =>
      ...
      val averages = for {
        ...
        lbestPSO = gbest(w, c1, c2, cognitive, guide)
        ...
        bests = for {
          ...
          syncGBest = Iteration.sync(lbestPSO)
